 ...
 * @package EndaLogView\LaravelLogViewer
 */
class LaravelLumenLogViewer
{
    /**
     * @var string file
     */
    ...
    public static function pathToLogFile($file)
    {
        ...
        $pathChangeSaveLog = env('PATH_SAVE_LOG');
        if (!empty($pathChangeSaveLog)) {
            $logsPath = dirname($pathChangeSaveLog);
        }
    ...
    public static function getFiles($basename = false)
    {
        ...
        $pathToLogFile = storage_path() . '/logs/' . $pattern;
        $pathChangeSaveLog = env('PATH_SAVE_LOG');
        if (!empty($pathChangeSaveLog)) {
            $pathToLogFile = dirname($pathChangeSaveLog).'/'.$pattern;
        }
        $files = glob($pathToLogFile , preg_match('/\{.*?\,.*?\}/i', $pattern) ? GLOB_BRACE : 0);
